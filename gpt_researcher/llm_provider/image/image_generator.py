...
class ImageGeneratorProvider:
    ...
    def _ensure_client(self):
        ...
        if self._client is None:
            try:
                ...
                logger.info(
                    f"Initialized image generation with model: {self.model_name}")
            except ImportError:
                raise ImportError(
                ...
                logger.error(
                    f"Failed to initialize image generation client: {e}")
                raise
    ...
    def _crop_to_landscape(self, image_bytes: bytes, target_ratio: float = 16/9) -> bytes:
        ...
        try:
            ...
            logger.info(
                f"Cropped image from {width}x{height} to {width}x{new_height} (landscape)")
            return output.getvalue()

        except ImportError:
            logger.warning(
                "PIL not available for image cropping, returning original")
            return image_bytes
        except Exception as e:
                ...
                if response.candidates:
                    ...
                    if candidate.content and candidate.content.parts:
                        ...
                        for idx, part in enumerate(candidate.content.parts):
                            ...
                            logger.debug(
                                f"Part {idx}: inline_data={has_inline}, text={has_text}")
                    ...
                    for part in response.candidates[0].content.parts:
                        if hasattr(part, 'inline_data') and part.inline_data:
                            ...
                            generated_images.append({
                                ...
                                "url": web_url,  # Relative URL for markdown and frontend display
